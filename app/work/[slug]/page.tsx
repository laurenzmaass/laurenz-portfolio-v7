...
export default async function CaseStudyPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      {/* Case study header */}
      <header className="pt-28 pb-16 border-b border-border bg-surface">
        <Container>
          <FadeIn>
            <nav aria-label="Breadcrumb" className="mb-8">
              <ol className="flex items-center gap-2 list-none m-0 p-0">
                ...
                <li className="font-mono text-xs text-subtle" aria-hidden="true">
                  /
                </li>
                <li>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-6">
              ...
              {project.fictional && (
                <Tag className="text-muted italic">
                  fictional case study
                </Tag>
              )}
            </div>

            <h1 className="font-display italic text-[clamp(2.5rem,7vw,5rem)] leading-[1.0] tracking-[-0.03em] mb-4 max-w-[20ch]">
              {project.client}
            </h1>
            <p className="font-sans font-light text-xl text-muted mb-10 max-w-[48ch]">
              {project.title}
            </p>
        ...
        <div className="border-b border-border">
          <Container>
            <div className="py-6 flex flex-wrap items-center gap-3">
              <span className="font-mono text-xs text-muted uppercase tracking-[0.12em] mr-2">
                Stack
              </span>
              {project.stack.map((s) => (
                <Tag key={s}>{s}</Tag>
        ...
        <Container narrow>
          ...
          <Section title="The problem" label="01 / Problem">
            {project.problem.map((para, i) => (
              <p key={i} className="font-sans text-base text-ink/80 leading-relaxed mb-4 last:mb-0">
                {para}
              </p>
            ))}
          </Section>
          ...
          <Section title="Results" label="04 / Outcome">
            <div className="bg-surface p-6 mb-6 border-l-2 border-accent">
              <p className="font-display italic text-[2.5rem] leading-none tracking-[-0.02em] text-ink mb-1">
                {project.outcome.stat}
              </p>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border">
          {prev ? (
            <Link
              href={`/work/${prev.slug}`}
              className="group p-8 hover:bg-surface transition-colors duration-200"
            >
              <p className="font-mono text-xs text-muted mb-2">← Previous</p>
              <p className="font-sans font-medium text-base group-hover:text-accent transition-colors duration-150">
                {prev.client}
              </p>
              <p className="font-display italic text-sm text-muted mt-0.5">
                {prev.title}
              </p>
            </Link>
          ) : (
          ...
          {next ? (
            <Link
              href={`/work/${next.slug}`}
              className="group p-8 hover:bg-surface transition-colors duration-200 sm:text-right"
            >
    ...
    <FadeIn>
      <section className="py-12 border-b border-border last:border-0">
        <div className="mb-6">
          ...
          <h2 className="font-display italic text-2xl leading-tight tracking-[-0.01em]">
            {title}
          </h2>
        </div>
        {children}
